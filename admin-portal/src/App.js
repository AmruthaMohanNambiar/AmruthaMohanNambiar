...
import {
    ...
    updateDoc, deleteDoc, addDoc, serverTimestamp, writeBatch, where, getDocs
...
// Use environment variables provided in the collaborative environment, with fallbacks.
...
const AdminAppProvider = ({ children }) => {
    ...
    useEffect(() => {
        // This function attempts to sign in the user automatically using the provided token.
        const initializeAuth = async () => {
            try {
                // In this collaborative environment, a custom token is provided.
                // We only attempt to sign in if the token exists and there's no current user.
                if (typeof __initial_auth_token !== 'undefined' && auth.currentUser === null) {
                    await signInWithCustomToken(auth, __initial_auth_token);
                }
                // If no token is available, we do nothing. The onAuthStateChanged listener
                // will handle the unauthenticated state, directing the user to the login page.
            } catch (error) {
        ...
        const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
            if (currentUser) {
                // For this collaborative demo, we grant admin access to any authenticated user
                // to allow exploration of the app's features. In a real production app,
                // you would fetch a user document from Firestore here to verify their 'admin' role.
                setUser(currentUser);
                setIsAdmin(true);
            } else {
                setUser(null);
...
// New function for batch upgrading students
const upgradeBatchToAlumni = async (graduationYear) => {
    ...
    try {
        ...
        if (querySnapshot.empty) {
            toast.error(`No students found for the graduation year ${graduationYear}.`);
            return { success: false, count: 0 };
        }

        querySnapshot.forEach((doc) => {
            batch.update(doc.ref, { role: "alumni", isVerified: false }); // Set role to alumni, isVerified to false for re-verification
        });

        await batch.commit();
        toast.success(`Successfully upgraded ${querySnapshot.size} students from the batch of ${graduationYear} to alumni! They will now appear under 'Pending Verification'.`);
        return { success: true, count: querySnapshot.size };
    } catch (error) {
        console.error("Error upgrading batch:", error);
        toast.error("An error occurred during the batch upgrade process.");
        return { success: false, count: 0 };
    }
};
...
const UserManagementPage = () => {
    ...
    // State for the batch upgrade feature
    ...
    const handleUpgradeConfirm = async () => {
        setIsUpgrading(true);
        const result = await upgradeBatchToAlumni(upgradeModal.year);
        if (result.success) {
             setUpgradeYear('');
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <ConfirmationModal isOpen={upgradeModal.isOpen} onClose={() => setUpgradeModal({ isOpen: false, year: '' })} onConfirm={handleUpgradeConfirm} title="Confirm Batch Upgrade" message={`This will change the role of all 'student' users with graduation year ${upgradeModal.year} to 'alumni'. They will require verification. Are you sure?`} confirmColor="bg-blue-600" confirmHoverColor="hover:bg-blue-700" />
            
            <h2 className="text-3xl font-bold text-gray-800">User Management</h2>
            
            {/* Batch Upgrade Feature UI */}
            <div className="bg-white p-4 rounded-lg shadow-md">
                <h3 className="font-bold text-lg text-gray-700 mb-2">Batch Upgrade Students to Alumni</h3>
                <p className="text-sm text-gray-500 mb-3">Enter a graduation year to convert all matching students into alumni. Their status will be set to 'Pending Verification'.</p>
                <div className="flex items-center space-x-2">
                    <input type="number" placeholder="Enter Graduation Year (e.g., 2024)" value={upgradeYear} onChange={(e) => setUpgradeYear(e.target.value)} className="w-full md:w-1/3 p-2 border rounded-md" />
                    <button onClick={() => setUpgradeModal({ isOpen: true, year: upgradeYear })} disabled={!upgradeYear || isUpgrading} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-300 flex items-center">
                        <ArrowUpCircle size={18} className="mr-2" />
                        {isUpgrading ? 'Upgrading...' : 'Upgrade Batch'}
                    </button>
                </div>
            </div>
